 ...
 */
class GradientHostView(context: Context) :
  ...
  /** `host` prop. Descendants pick the nearest ancestor host whose name matches their `hostName`. */
  var hostName: String = DEFAULT_HOST_NAME
    set(value) {
      if (field == value) {
        return
      }
      ...
      if (isAttachedToWindow) {
        // Views under this host may now resolve to a different host (this one or an outer one).
        reattachGradientViews(this)
      }
    }

  private fun reattachGradientViews(view: View) {
    if (view is GradientView) {
      view.reattachToHost()
    }
    if (view is ViewGroup) {
      for (i in 0 until view.childCount) {
        reattachGradientViews(view.getChildAt(i))
      }
    }
  }
  ...
  companion object {
    const val DEFAULT_HOST_NAME = "main"
  }
}
